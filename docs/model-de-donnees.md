...
| Champ | Type | Notes |
|---|---|---|
| `role` | Char(32) | `manager`, `admin`, `super_admin` (décision 89 ; `dm` et `df` retirés par `accounts.0006_trois_roles` : comptes désactivés, passés `manager`, tracés dans `ChangeLog`) (migration `accounts.0002` : `owner` → `manager`, `country_manager` → `dm`, `controller` → `df`, `doo` → `super_admin`, `auditor` → `admin`) |
| `countries` | M2M → Country | le pays du `manager` — obligatoire à l'API comme dans `seed_users`. `admin` et `super_admin` sont toujours globaux : leurs pays sont vidés à l'enregistrement. Un `manager` sans périmètre ne voit **rien** |
...
Les trois rôles suivent l'organisation du groupe (décision 89). Côté
pays, un seul compte : le `manager` (Manager — pays) ouvre les dossiers
**de son pays**, saisit les lignes, dépose les pièces, soumet et importe ;
le référentiel de son pays (équipes, projets, intitulés, catégories,
bénéficiaires) est tenu par la RH. Côté siège : l'`admin` (Administrateur —
RH) **contrôle seul chaque dossier jusqu'à la clôture** — mise en contrôle,
justification ou refus, clôture, contrôle des pièces, réouverture,
décision sur une rectification — et tient les comptes, l'audit, le
référentiel de tous les pays, les enveloppes et les exports ; le
`super_admin` (DG, DO, CEO, DEV) **supervise** : il voit tout, relit le
journal d'audit et administre, sans déclarer ni contrôler. Ni l'un ni
l'autre ne crée de dossier ni ne dépose de justificatif. Le siège est
toujours global. Il n'y a plus de DM ni de DF, ni « direction des
opérations » ni « auditeur » distincts. Les droits sont des capacités
nommées (`accounts/permissions.py`, `CAPACITES`, décision 43), avec leurs
rôles par défaut et trois verrous (décision 89) : la déclaration
(`expenses.create`, `expenses.update`, `expenses.delete`, `proofs.upload`,
`dossiers.submit`) au `manager` seul ; le contrôle (`expenses.review`,
`expenses.validate`, `expenses.close`, `proofs.review`, `dossiers.reopen`,
`rectifications.decide`) à l'`admin` seul ; l'administration (`users.*`,
`configuration.manage`, `audit.read`, `budgets.*`, `reallocations.*`,
`rates.manage`, `data.export`, référentiel) aux deux administrateurs.
`data.import` est au pays par défaut, jamais au siège. Les administrateurs
règlent la matrice dans la configuration ; `/api/me/` la traduit en
capacités.

Un manager ne justifie jamais une dépense, et la règle des quatre yeux
reste en garde : celui qui a saisi une dépense ne la contrôle pas. L'adresse e-mail
...
| `phone` | Char(32) | téléphone ; **`phone` ou `email` exigé** à la création et à chaque modification (décision 93) |
| `email` | Email | e-mail ; voir `phone` |
| `contact` | Char(180) | « Autres coordonnées », texte libre, optionnel |
| `is_active` | Bool | |
...
- `review` : mise en contrôle par l'administrateur (`expenses.review`), facultative
  sauf si `require_review_step`.
- `justify` / `reject` : l'administrateur constate (`expenses.validate`) ; `reject`
  exige un motif et
...
- `reopen` : **réouverture** (`POST /api/dossiers/{id}/reopen/ {note}`),
  par `dossiers.reopen` (`admin` seul, jamais le pays ni le `super_admin`)
  ...
  **managers du pays** — ceux qui déclarent, donc qui ont à corriger —
  ...
  siège, qui n'a rien à refaire tant que le dossier n'est pas resoumis. Le
...
- `rectify` : **rectification d'un constat**, la seconde exception, là où
  ...
  tort (montant justifié faux, pièce prise pour une autre) : le pays ou
  le super administrateur le **demande** (`POST /api/rectifications/
  {expense, motif}`, `rectifications.request` : `manager`, `super_admin`
  par défaut — le pays voit l'erreur le premier ; l'`admin` ne
  trancherait pas sa propre demande, décision 89), une seule demande en attente par ligne
  (contrainte `rectification_une_en_attente_par_ligne`) ; un
  administrateur **décide** (`POST /api/rectifications/{id}/approve/` ou
  `/refuse/ {note}`, `rectifications.decide` : `admin` seul depuis la
  décision 89, jamais le pays) — **jamais l'auteur de la demande** (403), comme pour
...
audite, et la direction, qui supervise. Le pays n'y accède pas — le journal
relit les décisions de chacun.
...
| 18 | Rôles | **trois** : `manager`, `admin`, `super_admin` (décision 89 ; cinq jusque-là, avec `dm` et `df`). Ni direction des opérations ni auditeur distincts. Le pays déclare, l'administrateur contrôle et administre, le super administrateur supervise et administre. Enveloppes, réallocations, taux de change et dépassements : `budgets.*`, `reallocations.*`, `rates.manage` = `admin`, `super_admin` (décision 58) ; journal d'audit, exports : `admin`, `super_admin` ; réouverture : `admin`. Ces défauts se règlent dans la matrice des droits (décision 43), sauf les verrous |
...
| 20 | Réouverture | **seule exception à l'irréversibilité** : `dossiers.reopen` (`admin` seul depuis la décision 89, jamais le pays ni le `super_admin`), motif obligatoire (`Dossier.reopen_note`), `AuditLog` `reopened` sur le dossier et ses lignes, lignes en brouillon sans imputation, **notification aux `manager` du pays** (ceux qui déclarent et doivent resoumettre), pas au siège ; refusée dès qu'une ligne est justifiée ou clôturée. Pour demander des comptes, jamais pour corriger en silence |
| 21 | Fichiers | exports (`xlsx`, `csv`, `docx`, `pdf` ; `year`, `month` facultatif, `country`) **réservés à `data.export`** (`admin`, `super_admin` par défaut), lecture comprise. L'import Excel est une déclaration : **`data.import` au pays** (`manager` par défaut, jamais le siège, décision 89), dans son pays et ses équipes, page `/dossiers/import`. CSV UTF-8 avec BOM, séparateur `;` ; totaux à devise unique seulement. Chaque export et chaque import est journalisé avec ses paramètres |
...
| 26 | Siège et pays | **le manager déclare, l'administrateur contrôle, le super administrateur supervise** (décision 89). Le `manager` est le seul compte de pays ; `admin` et `super_admin` sont au siège, toujours globaux. La mise en contrôle (`expenses.review`) et le constat (`expenses.validate`) restent deux capacités distinctes, toutes deux à l'`admin`. La RH tient le référentiel de tous les pays |
...
| 60 | Le stockage ne bloque pas ce qui ne le concerne pas | **Le client S3 porte des délais bornés** (`client_config` dans `config.settings` : `AWS_S3_CONNECT_TIMEOUT` 3 s, `AWS_S3_READ_TIMEOUT` 10 s, `AWS_S3_MAX_ATTEMPTS` 2). Sans eux, botocore attend soixante secondes par tentative et recommence jusqu'à cinq fois. Un audit de résilience l'a mesuré : un stockage qui accepte la connexion sans jamais répondre — un pare-feu qui avale les paquets, pas un service arrêté — prenait les huit threads de gunicorn et rendait **toute** l'API muette, y compris les écrans qui ne touchent aucun fichier ; sans reprise, et sans une ligne de journal. Le `--timeout` de gunicorn ne rattrape rien : en mode `gthread` il surveille la boucle du worker, pas ses threads de requête. Après correction, le téléchargement rend 503 en trente secondes et le reste de l'API répond en vingt millisecondes pendant la panne. Tests : `config/tests/test_delais_stockage.py` **Le dépôt traduit la même panne que le téléchargement** : `ProofViewSet.create` rend un 503 « Le stockage des justificatifs ne répond pas » sur une exception du client S3 (`BotoCoreError`, `ClientError`), là où il rendait un 500 opaque — mesuré sur un banc à quatre étages avec un stockage objet bridé : 500 en 17,7 s avant, 503 en 16,8 s après, sans objet orphelin. `OSError` reste hors de cette famille : une trace d'audit impossible doit remonter, pas se déguiser en « réessayez ». Tests : `expenses/tests/test_stockage.py`, `docs/audit-resilience.md` §9. |
...
| 74 | Ce qui s'est passé depuis 02:00 se rattrape | **Les journaux de transaction sont archivés** (`deploy/archiver_wal.sh`, appelé par `archive_command`), une **sauvegarde physique** hebdomadaire leur sert de point de départ (`sauvegarder.sh`), et `deploy/restaurer_a_la_date.sh` rejoue les deux jusqu'à l'instant demandé. La chaîne de sauvegarde était sérieuse — dump quotidien chiffré, copie mensuelle gardée sans limite, miroir hors machine vérifié — mais sa seule granularité de reprise était le dump de 02:00 : **une panne de disque à 01:59 perdait toute la journée**, pour une application dont la raison d'être est de savoir où est la preuve. Éprouvé de bout en bout sur un banc de 72 Mo : sauvegarde physique en **3,0 s**, reprise à un instant précis en **0,6 s**, les 3 000 lignes effacées par erreur retrouvées et la bêtise absente. Un `pg_dump` **ne peut pas** servir de point de départ à une reprise — il est logique, elle exige un départ physique ; c'est pourquoi la sauvegarde physique est obligatoire et surveillée à son propre seuil (huit jours, puisqu'elle est hebdomadaire). **Le mode de panne à connaître** : tant que l'archivage échoue, Postgres conserve ses segments dans `pg_wal`, où ils s'accumulent jusqu'à remplir le disque de la base — il préfère s'arrêter que perdre. `verifier_sauvegardes` compare donc les deux horodatages de `pg_stat_archiver` et prévient dès qu'un échec est postérieur au dernier succès ; il n'alerte pas sur l'ancienneté du dernier segment, qu'une nuit sans écriture rendrait bruyante tous les week-ends. La purge se cale sur l'étiquette `.backup` de la plus ancienne sauvegarde physique gardée (`pg_archivecleanup`) : trop peu purger remplit le disque, trop purger rend l'archive inutilisable. Le mode par défaut de la reprise est **l'essai** — répertoire jetable, base temporaire à côté, production intacte : une répétition qui détruit la production ne se répète qu'une fois. Tests : `core/tests/test_archivage_des_journaux.py`, `reporting/tests/test_sauvegardes.py` (`ArchivageDesSegmentsTests`) |
| 75 | Une seconde machine suit, sans pouvoir tuer la première | **Réplique en attente chaude** : `deploy/docker-compose.replique.yml` (surcharge qui décide ce qui tourne, sans redéfinir aucun service), `preparer_replique.sh` (copie initiale, emplacement de réplication) et `promouvoir_replique.sh` (bascule). Éprouvé sur banc, deux grappes locales : retard **0 octet / 0,69 ms**, copie initiale de 15 Mo en **0,5 s**, promotion en **0,11 s**, et **aucune perte** après un SIGKILL de la primaire — l'écriture d'avant sa mort était sur la réplique. **La borne `max_slot_wal_keep_size` est le cœur de la décision** : un emplacement demande à la primaire de garder ce que la réplique n'a pas lu, et une seconde machine absente ferait grandir `pg_wal` jusqu'à remplir le disque — mesuré. Avec la borne, l'emplacement passe à `lost`, la primaire **continue de servir**, et c'est la réplique qu'on refait : perdre la réplique vaut mieux que perdre la base. Réplication **asynchrone**, assumée : en synchrone, une réplique absente bloquerait toute la plateforme, remède pire que le mal avec une seule réplique — le prix est que quelques transactions validées peuvent être perdues, et c'est l'archivage (décision 74) qui couvre ce reste. **Une réplique n'est pas une sauvegarde** : un `DELETE` se réplique en moins d'une milliseconde ; elle protège de la perte d'une machine, l'archivage de l'erreur humaine. Sur la réplique, les services de sauvegarde restent à l'arrêt (profil `bascule`) : deux machines écrivant dans le même coffre distant s'écraseraient. `archive_mode=on` y reste sans dommage — Postgres n'archive pas en attente, il s'y met à la promotion. La promotion **refuse de s'exécuter si la primaire répond encore**, et rappelle les deux gestes qu'aucun script ne fait : le domaine, qui pointe encore ailleurs et où passe l'essentiel de l'indisponibilité réelle, et l'ancienne machine, qui ne redémarre jamais en primaire. La surveillance (`verifier_sauvegardes`) signale un emplacement **inactif** ou **perdu** — sans quoi l'on croit avoir une réplique jusqu'au jour de la bascule. **Non éprouvé** : deux machines réelles, un vrai réseau, et la bascule du domaine. Tests : `core/tests/test_replique.py`, `reporting/tests/test_sauvegardes.py` (`RepliqueTests`) |
| 76 | Le point de santé dit si la machine est la bonne | **`/api/health/` ne répond 200 que si la base accepte les écritures** (`pg_is_in_recovery()`), et expose `writable` ; une réplique rend 503 avec `status: replique`. Devant les deux machines, un **aiguillage** (`deploy/balanceur/Caddyfile`, `docker-compose.balanceur.yml`) envoie tout à celle qui se déclare primaire — `lb_policy first` plus ce contrôle actif. Ce n'est **pas** une répartition de charge : les deux machines ne sont pas interchangeables, la seconde porte une base en lecture seule, et partager le trafic refuserait la moitié des enregistrements. Ce que cela apporte est précis : sans aiguillage, une bascule oblige à changer le DNS et c'est le TTL du cache qui décide du retour du service — **c'est là que passe l'essentiel de l'indisponibilité réelle**, pas dans la promotion, qui prend 0,11 s. Ce qui le rend sûr est le contrôle d'écriture : une réplique répond parfaitement au `SELECT 1`, et pendant les 17 s où la primaire était perdue sans être remplacée, la réplique n'a reçu aucune requête. **Ce qu'il ne protège pas, et la première version de cette décision l'affirmait à tort** : l'ancienne primaire qui redémarre après une bascule. Rallumée telle quelle, elle n'est pas en récupération, elle accepte les écritures, `/api/health/` y répond 200 en toute sincérité, et `lb_policy first` lui rend le trafic — mesuré : **5,1 s** après son retour, 22 requêtes sur 24, sur une base périmée. Le contrôle répond à « puis-je écrire ? », pas à « suis-je la primaire d'aujourd'hui ? », et aucun signal local ne les distingue. La protection est une consigne d'exploitation — une machine perdue ne redémarre jamais telle quelle —, écrite dans `promouvoir_replique.sh` et tenue par un test ; départager deux bases demanderait un arbitre extérieur, non justifié (`docs/audit-resilience.md` §9.5). Derrière l'aiguillage, trois réglages doivent être vrais ensemble (`docker-compose.derriere-balanceur.yml`), et **deux cassent en silence** : `DJANGO_NUM_PROXIES=3`, sans quoi `client_ip` lit l'adresse de l'aiguillage pour tout le monde et la limite anti-bourrage (décision 68) devient un seul compteur commun ; `MANDATAIRES_DE_CONFIANCE`, sans quoi Caddy réécrit `X-Forwarded-Proto` et Django redirige en boucle ; `ADRESSE_PRIVEE`, sans quoi la machine publie encore sur l'Internet. **Une IP flottante chez l'hébergeur est préférable** quand elle existe : rien à tenir, et elle n'a pas le point de défaillance unique que l'aiguillage introduit. **Non éprouvé** : Caddy n'était pas disponible sur le banc, la configuration a été écrite et relue, pas exécutée ; le contrôle de santé, lui, est testé. Tests : `core/tests/test_health.py`, `core/tests/test_balanceur.py` |
| 77 | Savoir *pourquoi*, sans emporter le dossier | **Sentry, éteint par défaut** (`backend/config/surveillance.py`, `SENTRY_DSN`). Grafana et Prometheus disent *que* la plateforme va mal ; ils ne disent pas *pourquoi*, et une trace d'exception part à la rotation des journaux du conteneur. Sans `SENTRY_DSN`, rien ne s'installe et rien ne sort : le renseigner est une décision, car une donnée part alors chez un tiers. **Ce qui ne part pas est réglé, pas promis** : `send_default_pii=False`, `max_request_body_size="never"` (le corps porte montants, bénéficiaires et jusqu'à 20 Mo de pièce), `include_local_variables=False`, fils d'Ariane au-dessus de `INFO`, et un `before_send` qui retire les en-têtes portant l'adresse du client, le compte et l'adresse que `core.journalisation` attache à chaque ligne, et les arguments de la ligne de commande. **Les trois derniers ont été ajoutés après mesure** : un événement réel capté sur un banc portait `uploaded_by='manager.banc'` dans les variables locales, `extra.compte`, et l'adresse dans `X-Forwarded-For` — les réglages seuls n'y suffisaient pas (`docs/audit-resilience.md` §9). L'interface n'est pas surveillée : la politique de sécurité du contenu n'autorise que l'origine (`connect-src 'self'`), et l'ouvrir à un tiers — ou monter un relais — se décide. Tests : `config/tests/test_surveillance.py` |
| 78 | Ce qui permet la reprise part aussi hors machine | **Les segments archivés et les sauvegardes physiques sont copiés hors machine** par `sauvegarde-distante` (`sauvegarder.sh`, familles `base base-physique wal pieces` ; préfixes `wal/` et `physique/` du distant), et **chaque famille pose son marqueur** (`distant`, `distant-pieces`, `distant-wal`, `distant-base-physique`). Avant : la boucle ne connaissait que la base et les pièces, `demande-wal` n'était jamais consommée, et trois documents affirmaient le contraire — la reprise à un instant donné (décision 74) ne survivait qu'à la perte d'un disque, pas à celle du serveur, pendant que le RPO annoncé était de quelques minutes. `verifier_sauvegardes` confronte désormais chaque réussite locale à sa copie (`retards_de_copie`, marge de deux heures ; les segments par la date du plus récent) et notifie un retard, sans redire quatre fois l'absence de distant. Tests : `deploy/tests/test_sauvegarder.sh`, `reporting/tests/test_sauvegardes.py` (`RetardDeCopieTests`), `core/tests/test_archivage_des_journaux.py` |
| 79 | Le volume des sauvegardes appartient à qui y archive | **`sauvegardes-init` donne le volume à `postgres` avant que la base ne démarre**, `sauvegarde` tourne sous `postgres`, et la reprise a son service, `reprise` (profil, jamais lancé par `up`), seul à voir `pgdata` avec la base. Docker crée un volume vide en root ; Postgres archive sous `postgres` : `mkdir base/wal` échouait au premier segment, l'archivage échouait à chaque essai, et les segments s'accumulaient dans `pg_wal` jusqu'à remplir le disque — visible seulement au contrôle du lendemain. Le script de reprise, lui, n'était monté nulle part, et le service qui devait le lancer tournait en root, que `pg_ctl` refuse, sans voir le répertoire de données ; ce répertoire est un point de montage qui ne se renomme pas, l'ancien contenu est mis de côté dans `base/avant-reprise-<horodatage>/`. Le mode essai se suffit : base ouverte dans le conteneur, instant d'arrêt lu dans le journal, comptes, `--requete`, puis jetée. La clé privée n'est pas dans `.env` : apportée sur la ligne de commande le jour venu. **La CI joue la chaîne sur la pile livrée** (`ci.yml`, « Pile de production ») : segment archivé sans échec, sauvegarde physique, effacement, reprise à l'instant d'avant, 1 000 lignes retrouvées. Sa première exécution a trouvé un quatrième défaut : `pg_basebackup` ouvre une connexion de **réplication**, que le `pg_hba.conf` de l'image n'autorise que depuis la machine elle-même — la sauvegarde physique hebdomadaire échouait depuis le service `sauvegarde`, chaque semaine, dans la pile livrée. `deploy/initdb/10-sauvegarde-physique.sh` ajoute `host replication <propriétaire> samenet scram-sha-256` à l'initialisation d'un cluster neuf ; un cluster déjà en service reçoit la ligne à la main, une fois (`deploy/README.md`), et `sauvegarder.sh` imprime cette commande quand la base refuse. Tests : `core/tests/test_archivage_des_journaux.py` (`LaRepriseEstCableeTests`) |
| 80 | La réplique se prépare et se promeut là où les outils sont | **Le port 5432 de la primaire n'est publié que par `docker-compose.primaire.yml`, sur `ADRESSE_PRIVEE`** ; la copie initiale se fait **dans un conteneur de l'image de la base** (`run --rm --entrypoint /preparer_replique.sh db`, script monté par `docker-compose.replique.yml`, relancé sous `postgres` après avoir donné le volume) ; la promotion passe par **`select pg_promote(true, 60)`** et `pg_isready` par `compose exec`. Avant : le port n'était publié nulle part, la préparation exigeait `pg_basebackup` sur l'hôte et écrivait dans un répertoire de l'hôte au lieu du volume, et `pg_ctl promote` par `exec` entrait en root, que `pg_ctl` refuse — la bascule aurait échoué le jour où l'on ne peut pas la déboguer. La répétition sur deux machines réelles reste à faire (`docs/audit-resilience.md` §10). Tests : `core/tests/test_replique.py` |
| 81 | Un droit retiré ne survit pas à une coupure de Redis | **La configuration en cache expire au bout de 60 s (`WorkflowConfiguration.CACHE_TTL`) et `oublier()` efface Redis et le secours.** Avant : entrée sans expiration, un seul `delete` — Redis injoignable à l'instant d'un `PATCH /api/permissions/`, le `delete` tombait sur le secours, l'ancienne matrice restait dans Redis à son retour, et un compte gardait jusqu'au redémarrage un droit que le journal disait retiré. Coût : une lecture par minute et par processus. Tests : `core/tests/test_cache_de_secours.py` (`ConfigurationSousPanneDeRedisTests`) |
| 82 | Le RPO est écrit à un seul endroit | **`deploy/README.md`, tableau « Objectifs »** : 5 minutes tant que les segments partent hors machine, 24 h si leur copie est en défaut et que l'alerte n'est pas lue. `docs/infra-base-de-donnees.md` y renvoie. Avant : 24 h dans `deploy/README.md` et la décision 51, « quelques minutes » et « le chiffre n'est écrit nulle part » dans `docs/infra-base-de-donnees.md` — deux valeurs, toutes deux fausses tant que les segments ne partaient pas |
| 83 | Données personnelles : ce qui est conservé sans limite, dit tel quel | **Question posée, non tranchée par le code.** « Rien ne se supprime, rien ne se purge » vaut aussi pour l'adresse IP et l'agent utilisateur des journaux (`ChangeLog.ip_address`, `AuditLog`), les échecs de connexion, les notifications, les adresses e-mail des comptes. Défendable pour le journal d'audit, dont la valeur probante exige l'intégralité ; moins évident pour l'adresse d'un `login_failed`. Le cadre légal applicable aux dix-sept filiales (Sénégal loi 2008-12 / CDP, Côte d'Ivoire ARTCI, etc.), le registre des traitements et une éventuelle durée pour les seules traces techniques sont une décision de la direction, à consigner ici quand elle sera prise. Aucune purge n'est codée en attendant |
| 84 | Deux images par service sur le serveur, et un disque surveillé sans Grafana | **`deploy.sh` retire les images étiquetées autres que celle en ligne et la précédente** (`menage_des_images`) ; `image prune` ne retirait que les couches orphelines et chaque livraison de `main` ajoutait deux images sur le disque que partagent la base, ses segments et les sauvegardes. Le retour arrière manuel au-delà se fait depuis le registre, avec un jeton `read:packages` sur l'entrée standard (README, « Revenir en arrière »). **`verifier_sauvegardes` lit l'espace libre du volume** (`shutil.disk_usage`) et notifie sous `SAUVEGARDES_DISQUE_MIN_POURCENT` (15 %) : l'alerte disque de Grafana vit dans un profil désactivé par défaut, et un archivage cassé remplit précisément ce disque. Tests : `reporting/tests/test_sauvegardes.py` (`EspaceDisqueTests`) |
| 85 | La CI voit l'exploitation | **`shellcheck --severity=error` sur `deploy/`**, **Dependabot `docker-compose` sur `/deploy` et `/`** (Postgres 16.4 datait d'août 2024 et personne n'en était averti), et le travail « Pile de production » (décision 79). Les scripts de reprise, de réplique et de livraison ne tournaient dans aucune CI ; `sh -n` ne voyait qu'une syntaxe invalide |
| 86 | La double authentification s'impose par rôle | **`DJANGO_TOTP_REQUIRED_ROLES`** (liste, vide par défaut ; un rôle inconnu refuse le démarrage) ferme la plateforme aux seuls rôles nommés non enrôlés ; `DJANGO_TOTP_REQUIRED=1` vaut pour tous. Une seule fonction tranche, `accounts.middleware.totp_exige_pour`, pour le verrou, l'admin Django et `GET /api/me/` (`totp_required` devient la politique **de ce compte**). CLAUDE.md recommandait d'imposer le second facteur aux comptes privilégiés avant l'ouverture aux filiales « comme réglage de déploiement » : ce réglage n'existait pas, imposer aux administrateurs imposait aux dix-sept filiales. Tests : `accounts/tests/test_totp_par_role.py` |
| 87 | Le jeton vit dans `localStorage`, et c'est un choix | **`frontend/src/lib/api.ts`** : l'API est par jeton (DRF `TokenAuthentication`), sans cookie de session, donc sans surface CSRF ; le prix est qu'un script injecté lirait le jeton, ce que la politique de sécurité de contenu de `frontend/nginx.conf` (`script-src 'self'`) et l'absence de tout `dangerouslySetInnerHTML` tiennent à distance. Un cookie `HttpOnly` reviendrait à gérer CSRF et sessions côté serveur ; à reconsidérer si l'application devait servir des origines tierces. Consigné parce que rien dans `docs/` ne le disait |
| 88 | Le courrier est coupé par défaut | **Demande de la direction (23 septembre 2026) : aucun e-mail vers les utilisateurs.** `settings.EMAIL_ENABLED` (`DJANGO_EMAIL_ENABLED`, `0` par défaut) : coupé, `notify` écrit la notification sans programmer d'e-mail, `envoyer_les_emails` ne réclame rien (aucun essai brûlé, `emailed_at` jamais posé : rien ne prétend être parti), `send_periodic_report` s'arrête en le disant (`--dry-run` reste permis), et le transport devient `core.courrier.CourrierCoupe`, qui n'envoie rien et le journalise — filet pour tout envoi hors de ces deux chemins. `EMAIL_HOST` n'est plus exigé tant que le courrier est coupé et ne le rouvre pas s'il reste renseigné. *Configuration › Général* affiche « Coupé » (`notifications.email_actif`). Les notifications restent la trace in-app de chaque événement. Rouvrir : `DJANGO_EMAIL_ENABLED=1`, sur décision de la direction ; la reprise enverrait alors les notifications de moins de trois jours (`AGE_MAX_DE_REPRISE`). Tests : `config/tests/test_email.py`, `notifications/tests/test_notifications.py` (`CourrierCoupeTests`), `reporting/tests/test_commandes.py` (`RapportCourrierCoupeTests`) ; les tests d'envoi ouvrent l'interrupteur explicitement |
| 89 | Trois rôles : le pays déclare, l'administrateur contrôle, le super administrateur supervise | **Demande de la direction (23 septembre 2026).** Les rôles `dm` et `df` disparaissent ; le contrôle qu'ils se partageaient revient à l'`admin`, **seul**, jusqu'à la clôture. Ni `admin` ni `super_admin` ne créent de dossier ni ne déposent de justificatif ; le `super_admin` supervise (lecture de tout, audit, administration) sans déclarer ni contrôler. Trois verrous dans `accounts/permissions.py` : déclaration (`expenses.create`, `expenses.update`, `expenses.delete`, `proofs.upload`, `dossiers.submit`) fixe au `manager`, fermée au siège ; contrôle (`expenses.review`, `expenses.validate`, `expenses.close`, `proofs.review`, `dossiers.reopen`, `rectifications.decide`) fixe à l'`admin`, fermé au pays et au `super_admin` ; administration aux deux administrateurs (décision 58). `Capacite.fixes` n'est donc plus toujours « les administrateurs ». **Un dossier appartient à un pays** : attribué à la création, il ne change plus jamais de pays, même vide (`DossierSerializer._verifier_le_deplacement`) ; la liste des dossiers se filtre par pays (`?country=`). Un brouillon ne se modifie et ne se soumet que par son auteur, plus par le siège (`workflow.agit_en_auteur`). **L'import devient une déclaration** : `data.import` au `manager`, jamais au siège, dans son pays et ses équipes, sans créer d'équipe ni de responsable sans le droit correspondant sur le référentiel, jamais dans le brouillon d'un collègue ; sa page quitte la configuration pour `/dossiers/import`. Le siège est toujours global : ses pays sont vidés à l'enregistrement ; un manager doit avoir son pays. **Reprise** : `accounts.0006_trois_roles` passe les comptes `dm`/`df` au rôle `manager` et les **désactive**, sans leur donner les droits d'un administrateur, avec une entrée `ChangeLog` par compte (ancien rôle) — un administrateur décide ensuite de leur sort ; `expenses.0017_brouillons_du_siege_rendus_au_pays` vide l'auteur des brouillons ouverts par le siège — y compris ceux des anciens DM et DF, reconnus par l'historique de la reprise précédente — pour que le pays les finisse, avec une entrée `AuditLog` par objet ; `accounts.0007_matrice_trois_roles` retire `dm` et `df` de la matrice réglée et rend à `data.import` et `rectifications.request` leur nouveau défaut (un réglage « administrateurs » sur l'import, une fois les verrous appliqués, ne laisserait plus personne importer), tracé dans `ChangeLog`. **Brouillons sans auteur** : celui qui soumet un brouillon sans auteur connu en devient l'auteur, dossier et lignes (`transitions._soumettre_les_lignes`) — sans quoi la règle des quatre yeux resterait invérifiable et la ligne ne se contrôlerait jamais. **Un brouillon déjà déclaré puis rouvert ne se supprime plus**, même sans auteur (`workflow.a_ete_declare` : `reopen_note` pour un dossier, une entrée `AuditLog` `submitted` pour une ligne) : il se corrige et se resoumet. **Seul l'auteur d'un brouillon y ajoute une ligne** (`add_line`, `ExpenseViewSet.perform_create` et `perform_update`), comme il est seul à le modifier. **La demande de rectification** passe à `manager` et `super_admin` : l'`admin`, seul à trancher, ne trancherait pas sa propre demande, et le siège peut n'en compter qu'un ; la matrice la lui rouvre quand ils sont deux. Conséquences assumées : la politique « approbation » d'un dépassement se valide par l'administrateur en justifiant, puisqu'il tient aussi les enveloppes ; les soumissions ne préviennent plus le `super_admin`. Tests : `accounts/tests/test_matrice_des_droits.py`, `expenses/tests/test_actions_autorisees.py`, `expenses/tests/test_workflow.py` (`SeparationOfDutiesTests`), `reporting/tests/test_import.py`, `expenses/tests/test_coherence_dossier.py`, `expenses/tests/test_migration_trois_roles.py`, `expenses/tests/test_reouverture.py`, `expenses/tests/test_rectification.py` |
| 90 | Préproduction et production : deux serveurs | **Constat du 23 septembre 2026** : l'environnement GitHub `staging` portait les secrets du serveur de production, si bien que chaque fusion sur `main` y partait sans tag ni approbation — le code de la v1.1.0 était en ligne avant son tag. Deux piles sur une machine sont écartées (une seule tient les ports 80 et 443, et une préproduction qui sature disque ou mémoire ferait tomber la production). Désormais : chaque serveur porte une marque `ENVIRONNEMENT` (`staging` ou `production`, propriété de root, écrite par `preparer_serveur.sh` et `durcir_livraison.sh`, jamais réécrite si elle dit autre chose) ; `cd.yml` transmet l'environnement visé et `justi-livrer` refuse une livraison destinée à l'autre, ou tout serveur sans marque ; le déploiement en préproduction n'a lieu que si la variable de dépôt `PREPRODUCTION` vaut `1`, posée une fois le second serveur prêt, avec sa propre clé de livraison, son propre `.env` et aucune donnée de production. Ordre imposé : le serveur de production se met à jour avant la fusion (`deploy/README.md`, « Préproduction et production »). Tests : `deploy/tests/test_justi_livrer.sh` |
| 91 | Le super administrateur alloue, l'administrateur contrôle | **Demande de la direction (23 septembre 2026).** Les enveloppes passent au `super_admin` **seul** : `budgets.create`, `budgets.update`, la nouvelle `budgets.delete`, `reallocations.request`, `reallocations.decide`, `rates.manage` (`permissions._enveloppe` : défaut et fixe `super_admin`, verrouillées pour l'`admin` — qui règle la matrice et se les rouvrirait sinon — et pour le pays, sauf la demande de réallocation, qui reste ouvrable au pays). L'`admin` lit les enveloppes et leur consommation : il ne contrôle pas des dépenses imputées sur l'argent qu'il se serait alloué. **Conséquences assumées** : sous la politique « soumettre à approbation », l'`admin` ne peut plus justifier une dépense en dépassement et le `super_admin` ne justifie pas (décision 89) ; la dépense attend donc que la direction **abonde** l'enveloppe — montant relevé ou réallocation —, puis l'`admin` la justifie (`services.check_budget_capacity`) ; nul ne tranchant sa propre réallocation, il faut deux comptes `super_admin`. **Suppression d'une enveloppe** : seconde exception à « rien ne se supprime », après le brouillon — une enveloppe **jamais servie** (aucune dépense imputée, aucune réallocation qui la touche, aucune sous-enveloppe si c'est celle du pays) n'a pas plus de valeur probante qu'un brouillon jamais soumis ; `DELETE /api/budgets/{id}/` la supprime, sous verrou et après revérification (`budget.transitions.supprimer_enveloppe`, `raison_de_garder`), la trace s'écrit dans l'historique par le signal `post_delete` (qui, quand, depuis où). Une enveloppe qui a servi se désactive. `can_delete` sur chaque enveloppe (annotation `Budget.objects.with_usage`, une requête pour toute la liste). Pas de migration de la matrice : les verrous s'appliquent à la lecture. La recette gagne un second super administrateur (`recette.do`). Tests : `budget/tests/test_suppression.py`, `accounts/tests/test_matrice_des_droits.py`, `expenses/tests/test_workflow.py` (`OverrunPolicyTests`) |
| 92 | L'application s'appelle JUSTI GH | **Demande de la direction (25 septembre 2026).** Les filiales ne portent pas le nom d'INNOV PHARMA, qui est le siège : elles sont sous le couvert de **Generic Healthcare (GH)**. Le nom affiché devient « JUSTI GH » — interface (`BRAND.name`), titre de l'onglet, application installable (manifeste), libellé de la double authentification (`TOTP_ISSUER` : les comptes déjà enrôlés gardent l'ancien libellé dans leur application, leurs codes restent valides), titre du schéma d'API, tableau Grafana, documentation. Les **identifiants techniques ne changent pas** — conteneurs et projet Compose `justi-innov`, répertoire `/home/deploy/justi-innov`, images, dépôt, fichier du tableau Grafana — : renommer le projet Compose détacherait les volumes de la base. Le logo est celui de Generic Healthcare : le fichier fourni (`docs/identite/logo-gh.png`, 109 × 120 pixels, flou) a été vectorisé — couleurs séparées, contours lissés puis tracés en courbes (`docs/identite/logo-gh.svg`) — et `BrandLogo` écrit « JUSTI » devant le monogramme, ce qui se lit « JUSTI GH ». Ses trois couleurs ont des jetons réservés au logo (`--logo`, `--logo-rouge`, `--logo-orange`) ; le bleu passe au clair sur fond sombre. L'emblème « J » et sa source sont retirés. L'exemple d'identifiant (« prenom.nom ») ne suggère plus `.innov`. Le domaine des adresses (`ALLOWED_EMAIL_DOMAINS`, `innovpharma.net`) n'est pas touché : c'est un réglage de déploiement |
| 93 | Un bénéficiaire a un téléphone ou un e-mail | **Demande de la direction (25 septembre 2026).** Le champ « Contact », texte libre facultatif, devient **`phone`** et **`email`**, dont l'un est exigé (`BeneficiarySerializer.validate`, erreur générale) à la création et à **toute modification réelle** — une simple (dés)activation passe, même si l'écran renvoie tout le formulaire inchangé. Format vérifié : de 6 à 15 chiffres (E.164), séparateurs usuels et « + » admis, espaces normalisés (`expenses.contacts`) ; adresse e-mail valide. Pas de contrainte en base : les bénéficiaires saisis avant la décision restent utilisables et se signalent « À compléter » (`contact_manquant`, filtre `?contact_manquant=1`). Le texte libre reste, sous le nom « Autres coordonnées ». **Reprise** (`expenses.0018`) : un contact qui *est* un e-mail ou un numéro passe dans le champ correspondant, tout autre texte reste où il était — rien n'est perdu ni deviné. Tests : `expenses/tests/test_beneficiaires.py` (`ContactsTests`), `expenses/tests/test_migration_contacts.py` |
| 94 | MinIO se construit depuis ses sources | **Panne subie (25 septembre 2026).** MinIO a cessé de publier ses images : `quay.io/minio/minio` et `quay.io/minio/mc` refusent le téléchargement anonyme (`unauthorized`), `minio/minio` et `minio/mc` ont disparu de Docker Hub. La CI s'arrêtait au démarrage de la pile, et la livraison suivante aurait échoué au `compose pull` (la production en service gardait son image). Choix de la direction technique : **construire l'image nous-mêmes**, plutôt qu'une image communautaire, une archive figée ou un autre serveur S3. `deploy/minio/Dockerfile` compile **les versions exactes qui tournaient** (minio `RELEASE.2025-04-22T22-12-26Z`, commit `0d7408fc9969` ; mc `RELEASE.2025-04-16T18-13-26Z`, commit `b00526b153a3`) : le format du volume `miniodata` ne change pas. Les sources viennent du proxy de modules Go, vérifiées contre `sum.golang.org` ; la chaîne Go est la 1.25 (la 1.24 n'est plus maintenue), le reste est Alpine. Une seule image porte `minio` et `mc` : elle sert au service `minio` (contrôle de santé `mc ready local`) et à `sauvegarde-pieces` (`mc mirror`). **Étiquette** : `RELEASE.2025-04-22T22-12-26Z-<empreinte>`, les douze premiers caractères du `sha256` du Dockerfile ; la CI refuse une étiquette qui ne suit pas le fichier ou qui diffère entre `docker-compose.yml` et `deploy/docker-compose.prod.yml`. La livraison (`cd.yml`) publie l'image sur ghcr.io **seulement si l'étiquette est absente** : reconstruite, elle changerait d'empreinte et le conteneur des justificatifs serait recréé à chaque livraison. En développement et en CI, Compose la construit (`pull_policy: build`), deux à trois minutes la première fois. Monter une version de MinIO est une décision à part : vérifier d'abord les avis de sécurité publiés depuis avril 2025 (`govulncheck`, base `vuln.go.dev`, que l'environnement de développement ne joint pas), puis éprouver la nouvelle version en préproduction |
| 95 | La copie hors machine va chez Cloudflare R2 | **Choix de la direction (26 septembre 2026)**, à la place de Backblaze B2 (décision 51). Le mécanisme ne change pas — coffre rclone chiffré, vérification par `cryptcheck`, rétention par le bucket, le serveur n'efface rien de lui-même — ; le fournisseur, si. **Réglages** : `SAUVEGARDE_DISTANT_FOURNISSEUR=Cloudflare`, région `auto`, endpoint S3 du compte. Avec ce fournisseur, `sauvegarder.sh` pose `no_check_bucket` : une clé « Object Read & Write » ne peut ni vérifier ni créer un bucket, et rclone refuserait d'envoyer (documentation de rclone 1.72) ; le bucket se crée à la main. **Ce qu'on perd par rapport à B2** : les droits d'objet de R2 ne séparent pas écrire et supprimer — la clé du serveur peut effacer le distant — et R2 ne garde pas les versions antérieures d'un objet écrasé. **Parade** : le verrou de bucket (suppression et écrasement interdits, sans limite sur `mensuel/` et `pieces/`, 30 jours sur `quotidien/`, `wal/`, `physique/`), à poser avant la première copie ; sans lui, un serveur compromis peut emporter ses sauvegardes. Procédure : `deploy/README.md`, « Copie hors machine ». Test : `deploy/tests/test_sauvegarder.sh` (cas `distant-cloudflare`) |
...
|---|---|---|
| C1 | Validation par délégation au responsable pays | **Pas de délégation.** Le manager déclare, l'administrateur contrôle : un manager qui justifierait les dépenses de son pays viderait l'application de son objet. Les capacités de contrôle (`expenses.validate`, `expenses.review`, `expenses.close`, `proofs.review`, `dossiers.reopen`) sont verrouillées pour le rôle pays : la matrice configurable ne peut pas les lui ouvrir. |
...
| C5 | Six acteurs, dont une direction des opérations et un auditeur | **Trois rôles** (décisions 18 et 89) : la DO est super administratrice, l'audit revient à la RH. Deux rôles de plus, c'est deux listes de droits de plus à maintenir pour des personnes qui, dans le groupe, sont les mêmes. |
