...
| 39 | Cloisonnement : une seule primitive | **`accounts.perimetre.filtrer(queryset, access, pays=…, equipe=…)`** porte la règle — pays du compte, équipes du manager, une entité sans équipe (`team IS NULL`) échappe au manager cloisonné — et tout le reste lui délègue : `CountryScopedMixin`, `ChampCloisonne` (déplacé de `expenses` vers `accounts.perimetre`, avec `chemin_pays`, `chemin_equipe`, `distinct`), `Budget.objects.visible_par`, `reporting.scope.querysets_pour`. `PerimetreMixin` a disparu au profit de `ChampCloisonne`. `comptes_couvrant` en est la réciproque pour les notifications (qui voit un objet en est prévenu), vérifiée par un test. Les ensembles de rôles ne se déclarent que dans `accounts/permissions.py` (`COUNTRY_ROLES`, `HEADQUARTERS_ROLES` ré-exporté) : `CONTROLLERS`, `PROVIDERS`, `BUDGET_OWNERS`, `AUDIENCE`, `ACTION_ROLES` en sont des alias. Un test traversant (`accounts/tests/test_traversee.py`) parcourt chaque route du routeur avec un décor par pays et par équipe |
